...
const DichVuController = {
    ...
    createDichVu: async (req, res) => {
        try {
            const { TenDichVu, Gia, MoTa } = req.body;

            if (!TenDichVu || !Gia || !MoTa) {
                return res.status(400).json({ message: "Thiếu trường dữ liệu" });
            }

            const result = await DichVu.create({ TenDichVu, Gia, MoTa });
    ...
    updateDichVu: async (req, res) => {
        try {
            ...
            if (!TenDichVu || !Gia || !MoTa) {
                return res.status(400).json({ message: "Các trường TenDichVu, Gia là bắt buộc!" });
            }

            const result = await DichVu.update(id, { TenDichVu, Gia, MoTa });
